...
# 3.2 Small companies ---------------------------------------------------------
    ...
    dt_cit_small<-read_excel("Data/CIT/Micro/DBVP_2021.xlsx", 
                              sheet = "DV-VP-RAW")
    ...
    # Ensure all columns exist
    miss <- setdiff(cols_to_encode, names(dt_cit_small))
    if (length(miss)) stop("Missing columns in dt_cit_small: ", paste(miss, collapse = ", "))
    ...
    for (cl in cols_to_encode) {
      ...
      # Overwrite the original column in-place
      set(dt_cit_small, j = cl, value = codes)
    ...
    growth_factors_small<-read_csv("Data/CIT/growfactors_cit_small_macedonia.csv")%>%
      data.table()
