...
# I. ESTIMATION TAX LIABILITY FOR INCOME FROM LABOR --------------
                      ...
                      # )
                      # 
                      # # SAME code, now with explicit handling for STT==1 and STT==3 (set to 0)
                      # dt_scn[, ue_total_calc :=
                      #          fifelse(
                      #            (toggle_TIDZ == 1 & special_tax_treatment == 0) |
                      #              (toggle_TIDZ == 0 & special_tax_treatment == 2),
                      #            rowSums(.SD, na.rm = TRUE),
                      #            fifelse(special_tax_treatment %in% c(1, 3), 0, 0)  # explicit else-cases
                      #          ),
                      #        .SDcols = ue_cols]
                      ...
                      # 3) For every row: sum UE columns (ignore special_tax_treatment entirely)
                      dt_scn[, ue_total_calc := {
                        if (length(use_cols)) {
...
# 6. Decile ------------------------------------------------------------------
      ...
      # 
                      ...
                      # na_cols <- which(colSums(is.na(test)) > 0)
                      # names(test)[na_cols]
                      # 
                      # # Count NAs per column
                      # colSums(is.na(dt_cit))
                      # 
                      # # Rows that have any NA
                      # which(rowSums(is.na(dt_cit)) > 0)
                      # 
                      # 
                      # ###
                      # 
